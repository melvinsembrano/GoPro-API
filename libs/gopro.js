...
function GoProException(message) {
...
function GoPro(password, ip) {
    this._ip = ip || '10.5.5.9';
    this._request = request;
    this._password = password;
    this._apiUrl = 'http://' + this._ip;
    this._webUrl = this._apiUrl + ':8080';
}

// goProCall
// Performs GoPro query structured: http://10.5.5.9/[camera/bacpac]/<app>?t=[password]&p=<command>
GoPro.prototype.goProCall = function (device, method, parameter) {
    var dfd = when.defer();
    this._request(
        this._apiUrl + '/' + device + '/' + method + '?t=' + this._password + ((parameter !== undefined) ? '&p=%0' + parameter : ''),
        function (error, res) {
            if (error) {
...
// status
// Returns status of GoPro - reverse engineered
GoPro.prototype.status = function () {
    ...
    //Alternative approach to return Buffer
    http.get('http://10.5.5.9/camera/sx?t=' + that._password, function (resp) {
...
// Ready
// Resolves deferred object when the camera status is available.
// TODO: Fix Exception handling .... generally
GoPro.prototype.ready = function () {
    var that = this,
        dfd = when.defer(),
        t;
    t = setTimeout(function () {
        throw new GoProException('Cannot connect to cam');
    }, 5000);
...
// TODO: add defered to test ready && status on or off
GoPro.prototype.power = function (io) {
    ...
    this.goProCall('bacpac', 'PW', io).then(function(){
        function defPoll(){
...
// TIMELAPSE: http://10.5.5.9/bacpac/CM?t=[password]&p=%03 
GoPro.prototype.setCameraMode = function (mode) {
    if (GoPro.CameraMode.inEnum(mode)) {
        return this.goProCall('camera', 'CM', mode);
    }
};
...
// TIMELAPSE: http://10.5.5.9/bacpac/DM?t=[password]&p=%03 
GoPro.prototype.setDefaultCameraMode = function (mode) {
    if (GoPro.CameraMode.inEnum(mode)) {
        return this.goProCall('camera', 'DM', mode);
    }
};
...
GoPro.prototype.setVideoResolution = function (resolution) {
    if (GoPro.VideoResolution.inEnum(resolution)) {
        return this.goProCall('camera', 'VR', resolution);
    }
};
...
GoPro.prototype.setFieldOfVision = function (fov) {
    if (GoPro.FieldOfVision.inEnum(fov)) {
        return this.goProCall('camera', 'FV', fov);
    }
};
...
GoPro.prototype.setPhotoResolution = function (resolution) {
    if (GoPro.PhotoResolution.inEnum(resolution)) {
        return this.goProCall('camera', 'PR', resolution);
    }
};
...
GoPro.prototype.setPhotoTimer = function (timer) {
    if (GoPro.Timer.inEnum(timer)) {
        return this.goProCall('camera', 'TI', timer);
    }
};
...
GoPro.prototype.setVolume = function (volume) {
    if (GoPro.Volume.inEnum(volume)) {
        return this.goProCall('camera', 'BS', volume);
    }
};
...
GoPro.prototype.setLED = function (led) {
    if (GoPro.LED.inEnum(led)) {
        return this.goProCall('camera', 'LB', led);
    }
};
...
GoPro.prototype.preview = function (preview) {
    return this.goProCall('bacpac', 'PV', ((preview === undefined || preview) ? 2 : 1));
};
...
GoPro.prototype.orientUp = function (orient) {
    return this.goProCall('bacpac', 'UP', ((orient === undefined || orient) ? 0 : 1));
};
...
GoPro.prototype.locate = function (active) {
    return this.goProCall('camera', 'LL', ((active === undefined || active) ? 1 : 0));
};
...
GoPro.prototype.protune = function (active) {
    return this.goProCall('camera', 'PT', ((active === undefined || active) ? 1 : 0));
};
...
GoPro.prototype.spotMeter = function (active) {
    return this.goProCall('camera', 'EX', ((active === undefined || active) ? 1 : 0));
};
...
GoPro.prototype.oneButton = function (active) {
    return this.goProCall('camera', 'OB', ((active === undefined || active) ? 1 : 0));
};
...
GoPro.prototype.capture = function (active) {
    return this.goProCall('camera', 'SH', ((active === undefined || active) ? 1 : 0));
};
...
GoPro.prototype.onScreen = function (active) {
    return this.goProCall('camera', 'OS', ((active === undefined || active) ? 1 : 0));
};
...
GoPro.prototype.setVideoMode = function (mode) {
    if (GoPro.VideoMode.inEnum(mode)) {
        return this.goProCall('camera', 'VM', mode);
    }
};
...
GoPro.prototype.setAutoPowerOff = function (mode) {
    if (GoPro.AutoPowerOff.inEnum(mode)) {
        return this.goProCall('camera', 'AO', mode);
    }
};
...
GoPro.prototype.setWhiteBlack = function (mode) {
    if (GoPro.WhiteBlack.inEnum(mode)) {
        return this.goProCall('camera', 'WB', mode);
    }
};
...
GoPro.prototype.setLoopVideo = function (mode) {
    if (GoPro.LoopVideo.inEnum(mode)) {
        return this.goProCall('camera', 'LO', mode);
    }
};
...
GoPro.prototype.setContinuousShot = function (mode) {
    if (GoPro.ContinuousShot.inEnum(mode)) {
        return this.goProCall('camera', 'CS', mode);
    }
};
...
GoPro.prototype.setBurstRate = function (mode) {
    if (GoPro.BurstRate.inEnum(mode)) {
        return this.goProCall('camera', 'BU', mode);
    }
};
...
GoPro.prototype.deleteLast = function () {
    return this.goProCall('camera', 'DL');
};

// Delete All
// Technically formats the camera 
// TODO: verify settings are also formatted
GoPro.prototype.deleteAll = function () {
    return this.goProCall('camera', 'DA');
};
...
GoPro.prototype.listDirectory = function (path) {
    ...
    this._request(this._webUrl + (path || ''),
        function (e, res, body) {
            ...
            $.load(body)('#dirlist table tbody tr').each(function () {
                ...
                files.push({
                    ...
                    path: path,
                    absolutePath: that._webUrl + path,
...
GoPro.prototype.copyFile = function (fromPath, toPath) {
    ...
    when.resolve(this._request(fromPath)).then(function (src) {
